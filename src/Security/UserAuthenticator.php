    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $firewallName): ?Response
    {
        ...
        // }
            ...
            if (in_array('ROLE_STRUCTURE', $roles, true)) {
                $user = $token->getUser();
                $slug = $user->slug;
                return new RedirectResponse($this->router->generate("structures_details", ['slug' => $slug]));
            } else if (in_array('ROLE_PARTNER', $roles, true)){
                $user = $token->getUser();
                $slug = $user->slug;
                return new RedirectResponse($this->router->generate("partners_details", ['slug' => $slug]));
            } 
